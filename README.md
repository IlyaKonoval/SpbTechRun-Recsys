...
| Модель | Тип | NDCG@10 | MAP@10 | MRR | HR@10 |
|--------|-----|---------|--------|-----|-------|
| **MLP Ranker** | Neural | **0.725** | **0.725** | **0.725** | **0.725** |
| CatBoost (YetiRank) | GBDT | 0.724 | 0.724 | 0.724 | 0.725 |
| Embedding MLP | Neural | 0.723 | 0.723 | 0.723 | 0.725 |
| Formula Scoring | Heuristic | 0.720 | 0.725 | 0.725 | 0.725 |
| DCN v2 | Neural | 0.709 | 0.704 | 0.704 | 0.724 |
| FAISS Cosine | Retrieval | 0.463 | 0.453 | 0.455 | 0.502 |
| Popularity | Baseline | 0.038 | 0.026 | 0.031 | 0.077 |
| Random | Baseline | 0.036 | 0.024 | 0.029 | 0.076 |

**Нейросетевые модели (PyTorch):**
- **MLP Ranker** — 3-слойный MLP (128→64→32) на 39 табличных признаках. Лидер по NDCG
- **DCN v2** — Deep & Cross Network v2 (Google, 2020): автоматические cross-feature interactions + deep network
- **Embedding MLP** — проецирует сырые 768-dim эмбеддинги пары товаров через learned projection + 39 табличных фичей
...
- vs MLP Ranker: p = 0.083 n.s. (разница не значима — модели сопоставимы)
- vs DCN v2: p < 0.001 ***

> MLP и CatBoost статистически неразличимы — оба значимо превосходят все бейзлайны. LightFM не включён (не собирается на Windows).
...
> **Замечание:** 15 из 39 признаков имеют нулевой SHAP в offline evaluation — не потому что бесполезны, а потому что соответствующие данные недоступны офлайн:
> - **Контекст корзины** (3 фичи) — корзина недоступна в offline, в online-режиме вклад будет другим
> - **Сценарный фидбек** (4 фичи) — нет данных сценарного фидбека
> - **same_root_category, category_distance, embedding_has_valid, has_image** — константы при текущих данных (у всех товаров есть эмбеддинги и картинки)
...
|---------|----------|----------|
| NDCG@10 | **0.725** | MLP Ranker — лучший результат (offline eval) |
| Моделей сравнено | **8** | 2 baseline + 1 retrieval + 1 heuristic + 1 GBDT + 3 neural |
