...
#### Ensemble Ranker: ML-ранжирование
...
**Решение:** Ансамбль из трёх моделей — каждая делает разные типы ошибок, а их комбинация даёт лучшее качество:

| Модель | Тип | Loss | Сильные стороны |
|--------|-----|------|-----------------|
| **CatBoost** | GBDT | YetiRank | Табличные данные, устойчивость |
| **MLP Ranker** | Neural | BPR (pairwise) | Нелинейные комбинации признаков |
| **LightGBM** | GBDT | LambdaRank | Прямая оптимизация NDCG |

Веса ансамбля оптимизируются на валидации по NDCG@10.
...
**Обучение:**
```python
ensemble = EnsembleRankerService()
metadata = await ensemble.train_model(session)
# Обучает CatBoost + MLP + LightGBM
# Оптимизирует веса на валидации
# Сохраняет все модели + метаданные
```
...
Систематическое сравнение 8 моделей в 4 фазах: baseline → GBDT → нейросети → ансамбль. Подробности — в разделе [Исследование моделей ранжирования](#исследование-моделей-ранжирования).
...
| POST | `/ml/train` | Обучить Ensemble |
| GET | `/ml/model-info` | Статус модели |

---

## Исследование моделей ранжирования

### Методология

Для выбора оптимальной модели ранжирования проведено систематическое исследование в 4 фазы. Каждая фаза проверяет конкретную гипотезу, результаты подтверждены статистическими тестами.

**Условия эксперимента:**
- Данные: graded relevance (0/1/2) из пользовательского фидбека и совместных покупок
- Разбиение по `main_product_id`: 60% train / 20% val / 20% test
- Каждый test-запрос оценивается на пуле из 100 кандидатов
- Метрики: NDCG@10 (основная), MAP@10, MRR, HR@10
- Статистическая значимость: Wilcoxon signed-rank test (попарно по запросам)

### Фаза 1: Baseline → Gradient Boosting

**Гипотеза:** ML-модель, автоматически обучаемая на 39 признаках, превзойдёт ручную формулу скоринга с экспертно подобранными весами.

**Мотивация.** Первая версия ранжирования использовала формульный скоринг — линейную комбинацию `cosine_similarity`, `same_category` и `popularity` с вручную настроенными коэффициентами. Такой подход имеет три фундаментальных ограничения: (1) веса не оптимальны — подобраны интуитивно; (2) модель линейна — не может выразить нелинейные зависимости (скидка важнее для дорогих товаров); (3) не масштабируется — при добавлении новых признаков нужно вручную перенастраивать.

**Выбор модели.** CatBoost выбран среди GBDT-фреймворков (XGBoost, LightGBM, CatBoost) по двум причинам: (1) YetiRank — нативный listwise ranking loss, не требующий дополнительной настройки; (2) ordered boosting — снижает target leakage при обучении. В качестве бейзлайнов включены Random (нижняя граница), Popularity (non-personalized), FAISS Cosine (retrieval без ранжирования) и Formula Scoring (экспертная эвристика).

**Результаты:**

| Модель | Тип | NDCG@10 | p-value vs CatBoost |
|--------|-----|---------|---------------------|
| CatBoost (YetiRank) | GBDT | **0.724** | — |
| Formula Scoring | Heuristic | 0.720 | 0.008 ** |
| FAISS Cosine | Retrieval | 0.463 | < 0.001 *** |
| Popularity | Baseline | 0.038 | < 0.001 *** |
| Random | Baseline | 0.036 | < 0.001 *** |

**Анализ.** Гипотеза **подтверждена**. CatBoost значимо превосходит формульный скоринг (p = 0.008), хотя разница в абсолютных числах невелика (+0.004 NDCG@10). Это объяснимо: формула уже использует два наиболее важных сигнала (similarity и category), и CatBoost выигрывает за счёт нелинейных взаимодействий между признаками. Критически важно, что FAISS Cosine (чистый retrieval) отстаёт на 0.261 — это подтверждает необходимость двухуровневой архитектуры: FAISS для отбора кандидатов + ML для ранжирования.

### Фаза 2: Нейросетевые архитектуры

**Гипотеза:** Нейросетевые модели обучат более сложные нелинейные комбинации 39 признаков и превзойдут gradient boosting.

**Мотивация.** GBDT-модели строят ранжирование через поэтапное разделение пространства признаков (axis-aligned splits). Нейросети, в отличие от деревьев, обучают произвольные гиперплоскости и могут выразить мультипликативные взаимодействия признаков, которые деревьям требуют глубоких каскадов. Кроме того, для рекомендательных систем предложены специализированные архитектуры (DCN, AutoInt, DLRM), моделирующие cross-feature interactions в явном виде.

**Выбор архитектур.** Реализованы 3 модели на PyTorch, все обучены с BPR pairwise ranking loss, 80 эпох, CosineAnnealing scheduler:

1. **MLP Ranker** — 3-слойный MLP (256→128→64) с GELU-активацией, BatchNorm и Dropout 0.15 на каждом слое. Простейшая архитектура, верхняя граница для «чистого» обучения на табличных признаках.

2. **DCN v2** — Deep & Cross Network v2. Основная идея: explicit cross-network автоматически моделирует полиномиальные feature interactions (x₁·x₂, x₁·x₂·x₃, ...) без ручного feature engineering, параллельно с deep-сетью для неявных паттернов. Конфигурация: 3 cross-слоя + deep 256→128.

3. **Embedding MLP** — проецирует сырые 768-мерные эмбеддинги пары товаров (query, candidate) через learned projection в 64-мерное пространство, конкатенирует с 39 табличными признаками и ранжирует через MLP. Гипотеза: необработанные эмбеддинги содержат информацию, которую feature engineering (cosine_similarity и т.д.) теряет.

**Результаты:**

| Модель | Архитектура | NDCG@10 | p-value vs CatBoost |
|--------|-------------|---------|---------------------|
| **MLP Ranker** | MLP 256→128→64, GELU, BN, Dropout 0.15 | **0.725** | 0.083 n.s. |
| Embedding MLP | 768-dim projection + 39 фичей → MLP | 0.723 | — |
| DCN v2 | 3 cross layers + deep 256→128 | 0.709 | < 0.001 *** |

**Анализ.** Гипотеза **частично опровергнута**. MLP Ranker показал лучший абсолютный результат (0.725), но разница с CatBoost статистически незначима (p = 0.083 по Wilcoxon signed-rank test). Это согласуется с бенчмарками на табличных данных: GBDT конкурентоспособен с нейросетями на structured data с десятками-сотнями признаков, XGBoost/CatBoost остаются strong baseline.

DCN v2 показал неожиданно худший результат (0.709) — cross-network с 3 слоями на 39 признаках, вероятно, переобучился: полиномиальные interactions 3-го порядка на ограниченном объёме данных создают больше шума, чем полезного сигнала. Embedding MLP (0.723) не превзошёл чистые табличные признаки — ручной feature engineering (cosine_similarity, l2_distance и др.) уже извлекает ключевую информацию из эмбеддингов.

### Фаза 3: Выбор функции потерь

**Гипотеза:** Listwise loss (ApproxNDCG), напрямую оптимизирующий целевую метрику NDCG, даст лучший результат, чем pairwise BPR.

**Мотивация.** В learning-to-rank существует три класса функций потерь:
- **Pointwise** (MSE, Cross-Entropy) — предсказывает абсолютный скор, не учитывает порядок
- **Pairwise** (BPR, RankNet) — оптимизирует порядок между парами (pos, neg), но не учитывает позицию в списке
- **Listwise** (LambdaRank, ApproxNDCG, ListMLE) — оптимизирует метрику всего ранжированного списка

Теоретически listwise > pairwise > pointwise, так как listwise напрямую оптимизирует целевую метрику. ApproxNDCG аппроксимирует недифференцируемый NDCG через softmax-ранги, что позволяет использовать градиентный спуск.

**Эксперимент.** Замена BPR на ApproxNDCG в MLP Ranker. ApproxNDCG требует group-level оптимизации (loss считается по всем кандидатам одного query), в отличие от BPR, который работает на уровне mini-batch с shuffle.

| Loss | Тип | Подход к обучению | MLP NDCG@10 |
|------|-----|-------------------|-------------|
| BPR | Pairwise | batch-level, DataLoader shuffle | **0.9486** |
| ApproxNDCG | Listwise | group-level, итерация по query | 0.4286 |

**Анализ.** Гипотеза **опровергнута**. ApproxNDCG катастрофически деградировал (0.4286 — хуже random baseline). Причины:

1. **Малый размер групп.** В среднем ~3-4 примера на query. ApproxNDCG аппроксимирует ранжирование через softmax по списку — при 3 элементах аппроксимация крайне грубая, градиенты шумные. Для стабильной работы нужны списки из 50-200 документов.

2. **Потеря стохастичности.** BPR обучается на случайных mini-batch (512 примеров из разных query, shuffle=True), что обеспечивает хорошую регуляризацию. ApproxNDCG обучается последовательно по группам (2758 групп × 80 эпох) — нет стохастичности, медленная сходимость, переобучение на малых группах.

**Практический вывод:** BPR pairwise loss — предпочтительный выбор для нейросетевых рекомендательных моделей при малом числе кандидатов на запрос (<10). Listwise loss (ApproxNDCG, ListMLE) целесообразен при длинных списках (>50 кандидатов), что характерно для web search, но не для товарных рекомендаций.

### Фаза 4: Ансамблевое ранжирование

**Гипотеза:** Ансамбль моделей с разными inductive biases (GBDT + Neural) даст прирост за счёт разнообразия ошибок — каждая модель ошибается на разных примерах.

**Мотивация.** Ансамблевые методы — стандартный подход в ML-соревнованиях (Kaggle) и production-системах (Netflix Prize, рекомендации YouTube). Идея: если модели A и B имеют разные ошибки (низкая корреляция предсказаний), их комбинация снижает общую ошибку. В нашем случае CatBoost (tree-based, YetiRank) и MLP (neural, BPR) имеют фундаментально разные inductive biases, а добавление LightGBM (tree-based, LambdaRank) вносит разнообразие через другую функцию потерь.

**Эксперимент.** Weighted ensemble из 3 моделей. Веса оптимизированы grid search по NDCG@10 на валидационной выборке с шагом 0.1, перебор всех комбинаций (w₁, w₂, w₃) при условии w₁ + w₂ + w₃ = 1. Скоры моделей нормализованы в [0, 1] перед взвешиванием.

| Модель | Loss | NDCG@10 | Вес в ансамбле |
|--------|------|---------|----------------|
| CatBoost | YetiRank | 0.9488 | **0.6** |
| MLP Ranker | BPR | 0.9486 | 0.0 |
| LightGBM | LambdaRank | 0.9485 | **0.4** |
| **Ensemble** | — | **0.9488** | — |

**Анализ.** Гипотеза **не подтверждена на текущих данных**. Ансамбль не улучшил лучшую индивидуальную модель (0.9488 = CatBoost solo). Причины:

1. **Высокая корреляция предсказаний.** Все 3 модели показали практически идентичное качество (~0.9485-0.9488), что означает, что они делают одинаковые ошибки. Для эффективного ансамбля нужна корреляция предсказаний < 0.9 — здесь она близка к 1.0.

2. **Синтетический фидбек.** Текущие обучающие данные сгенерированы алгоритмически (на основе cosine similarity и категорий) — они содержат одни и те же паттерны, которые все модели легко выучивают. Реальный пользовательский фидбек вносит субъективность и шум, на котором модели начнут расходиться.

3. **MLP = 0 вес.** Нейросеть не добавляет информации поверх двух GBDT-моделей на текущих данных. Это ожидаемо: MLP и CatBoost обучены на идентичных 39 признаках, а MLP не имеет архитектурного преимущества перед деревьями на structured data (см. Фаза 2).

**Практический вывод:** Ансамблевая инфраструктура развёрнута в production (weighted ensemble с автоматической оптимизацией весов). При переходе на реальный пользовательский фидбек разнообразие ошибок между моделями вырастет, и ансамбль начнёт давать прирост — типичный gain в production-системах составляет +0.01-0.03 NDCG.
...
> Offline evaluation на данных из `evaluation_pipeline.ipynb`. Production ensemble обучается на полном датасете с другим разбиением.
...
1. **GBDT = Neural на табличных данных.** CatBoost и MLP статистически неразличимы (p = 0.083). На structured data с десятками признаков GBDT остаётся state-of-the-art
2. **Ranking loss критичен.** Все модели с ranking loss (YetiRank, BPR, LambdaRank) значимо превосходят pointwise-подходы. Выбор конкретного ranking loss менее важен, чем сам факт его использования
3. **Listwise loss не универсален.** ApproxNDCG теоретически оптимален, но требует длинных списков кандидатов (>10 на запрос). При малых группах pairwise BPR робастнее
4. **Ансамбль эффективен при разнообразии.** На синтетических данных модели слишком коррелированы; реальный фидбек должен раскрыть потенциал ансамбля
5. **Фидбек и категории > всего.** 74% вклада SHAP приходится на 2 группы из 7 — фидбек (40.9%) и категорийные признаки (33.1%)
...
|---------|----------|----------|
| NDCG@10 | **0.725** | Лучший результат (MLP Ranker, offline eval) |
| Моделей исследовано | **8** | 2 baseline + 1 retrieval + 1 heuristic + 1 GBDT + 3 neural |
| Гипотез проверено | **4** | GBDT vs baseline, Neural vs GBDT, Loss functions, Ensemble |
| Признаков | 39 | В 7 группах (SHAP + ablation study) |
| Inference | < 50ms | Время ранжирования 100 кандидатов |
| Production | Ensemble | CatBoost (0.6) + LightGBM (0.4) с оптимизацией весов |
...
| Рекомендации | < 200ms | FAISS + Ensemble |
...
│   │   ├── ml/                   # Ensemble (CatBoost+MLP+LightGBM), Feature Extractor
